...
```
daml build
daml codegen js .daml/dist/test-0.1.0.dar -o ui/daml.js
```
...
Deploying `test` to the hosted Daml platform
...
DAR `test.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/test-0.1.0.dar -o ui/daml.js
(cd ui && npm install && npm run-script build && zip -r ../test-ui.zip build)
```
...
(`test-ui.zip`) you have just created and publish it.

To see your deployed instance of `test` in action, follow the
...
- `daml.js/test-0.1.0/src/User.ts` (once you've generated it):
