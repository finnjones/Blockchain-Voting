...
```
daml build
daml codegen js .daml/dist/voting-0.1.0.dar -o ui/daml.js
```
...
Deploying `voting` to the hosted Daml platform
...
DAR `voting.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/voting-0.1.0.dar -o ui/daml.js
(cd ui && npm install && npm run-script build && zip -r ../voting-ui.zip build)
```
...
(`voting-ui.zip`) you have just created and publish it.

To see your deployed instance of `voting` in action, follow the
...
- `daml.js/voting-0.1.0/src/User.ts` (once you've generated it):
